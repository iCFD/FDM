function [L1_nodes,Linf_nodes] = TestFDMfun(CFL,tEnd,Ic,nc,method)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%   tEnd =  1.0;  % Final time
% method =    4;	% {1}Upwind,{2}TVD,{3}WENO3,{4}WENO5,{5}WENO7.
%     nc =   20;	% cells
...
% Time discretization
dt = CFL*dx/abs(u); tsteps = ceil(tEnd/dt); 
...
rk4c = [             0.0  ...
         ...
         2802321613138.0/2924317926251.0]; %#ok<NASGU>
...
for t = time
    ...
    switch method
        case 1  % Upwind
            ...
            q = qo-dt*FDM.Upwindresidual1d(q,u,dx); % cfl ~ 0.5
            
        case 2 % TVD
            
            % 1st stage
            %dF = FDM.TVDresidual1d(q,u,dt,dx,'Koren');
            %q = qo-dt*dF/2;
            ...
            q = qo-dt*FDM.TVDresidual1d(q,u,dt,dx,'Koren');

        case 3 % WENO3 % SSP-RK3-3-stages 

            % 1st stage
            dF = FDM.WENO3residual1d(q,u,dx);
            q = qo-dt*dF;
